...
class UltimateHumanizer:
    ...
    def __init__(self):
        ...
        # Use a shared, long-timeout HTTP client for all AI calls (essential for cloud platforms)
        ...
        if not GROQ_API_KEY:
            ...
            self.groq = AsyncGroq(api_key=GROQ_API_KEY, http_client=http_client)
            print("[OK] Groq Client Initialized with Timeout")
    ...
    async def humanize(self, raw_text: str, keywords: list[str] = None) -> dict:
        ...
        for i, chunk in enumerate(chunks):
            ...
            try:
                ...
                error_details = str(e)
                # Identify specific API errors to help the user
                if "401" in error_details:
                    reason = "[Unauthorized] Check your GROQ_API_KEY on Railway."
                elif "404" in error_details:
                    reason = "[Model Error] A model name was not recognized."
                else:
                    reason = error_details
                
                error_msg = f"Failed at chunk {i+1}: {reason}"
